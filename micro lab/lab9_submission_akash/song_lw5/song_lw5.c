...
sbit CS_BAR = P3^4;		//CS							// Chip Select for the ADC
...
unsigned int  voltage= 0, sending_voltage=0;
...
void main(void)
{
	P3 = 0x00;//TO CONFIGURE PORT3 AS OUTPUT
	CS_BAR = 1; //CHIP SELECT(CS ) 
	FS = 1;
	...
	//	if (0){
		...
		{
			sending_voltage=voltage;
		}
		
		
		data_lsb = sending_voltage;
		data_msb = sending_voltage>>8;
...
{//TO GENERATE THE HIGH FREQUENCY SQUARE WAVE SA RE GA MA PA
			TR1 = 0;
		
		TH1 = freq_high;   //for  240Hz
		TL1 = freq_low;	// for  240Hz
