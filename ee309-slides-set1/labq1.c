...
char * to_string(unsigned long val);
...
unsigned int avgVal=0, initVal=0, adcValue = 0;
...
void main(void)
{
	...
	while(1)
	{
		...
		//tempadcVal = data_save_high;
		LCD_CmdWrite(0x80);
		LCD_StringWrite(to_string(tempadcVal),4);
...
void timer0_ISR (void) interrupt 1
{
	...
	TR0 = 1;
	count = count + 1; //Increment Overflow 
